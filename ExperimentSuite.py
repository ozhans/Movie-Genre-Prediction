...
class ExperimentSuite:
    ...
    def train_model(self,layers, tbCallBack, train_x, train_y, test_x, test_y, loss, activation, epoch):
        ...
        model.add(tf.keras.layers.Dense(layers[0],activation))
        if(len(layers) >=2):
            model.add(tf.keras.layers.Dense(layers[1],activation))
        if(len(layers) >=3):
            model.add(tf.keras.layers.Dense(layers[2],activation))
        model.add(tf.keras.layers.Dense(len(train_y[0]),activation='softmax'))
        adam = tf.train.AdamOptimizer(learning_rate=0.0001)
        model.compile(optimizer=adam,loss=loss,metrics= metrics)
        model.fit(train_x, train_y, epochs=epoch,callbacks=[tbCallBack],verbose=0)
